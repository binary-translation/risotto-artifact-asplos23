...
for b in ${benchmarks[@]}; do
    # native
    ${TASKSET} ./a2a-benchmarks/bench.py  -b parsec.$b -d native -r native -o results/results.csv -a aarch64 -n $(nproc) -i ${NR_RUNS} -t native -c configs/native.config -vvv

    # QEMUs
    for q in master-6.1.0 no-fences tcg-tso risotto; do
	${TASKSET} ./a2a-benchmarks/bench.py  -b parsec.$b -d native -r qemu -o results/results.csv -a x86_64 -n $(nproc) -i ${NR_RUNS} -t $q -c configs/qemu-${q}.config -vvv
    done
done
