...
# QEMUs
${TASKSET} ./a2a-benchmarks/bench.py -b micro.sqlite -d native -r qemu -o ${OUTPUT} -a x86_64 -n $(nproc) -i ${NR_RUNS} -t master-6.1.0 -c configs/qemu-master-6.1.0.config -vvv
${TASKSET} ./a2a-benchmarks/bench.py -b micro.sqlite -d native -r qemu --run-opt='-nlib configs/nlib/sqlite.nmi' -o ${OUTPUT} -a x86_64 -n $(nproc) -i ${NR_RUNS} -t risotto -c configs/qemu-risotto.config -vvv
