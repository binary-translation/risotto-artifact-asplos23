...
# QEMUs
${TASKSET} ./a2a-benchmarks/bench.py -b micro.math -d native -r qemu -o ${OUTPUT} -a x86_64 -n $(nproc) -i ${NR_RUNS} -t qemu -c configs/qemu.config -vvv
${TASKSET} ./a2a-benchmarks/bench.py -b micro.math -d native -r qemu --run-opt='-nlib configs/nlib/math.nmi' -o ${OUTPUT} -a x86_64 -n $(nproc) -i ${NR_RUNS} -t risotto -c configs/risotto.config -vvv
